...
async def lifespan(app: FastAPI):
    ...
    yield
    logger.info("[SHUTDOWN] RAG Backend Server shutting down...")
...
async def ingest(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    try:
        ...
        logger.info(f"[OK] File read successfully: {len(file_bytes)} bytes")
    ...
    logger.info("[OK] Ingestion task queued successfully")
