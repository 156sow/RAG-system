...
def ingest_document(file_bytes: bytes, filename: str, session_id: str = None):
    ...
    try:
        ...
        logger.info(f"[OK] Text loaded successfully: {len(text)} characters")
        if session_id:
            progress_tracker.add_step(session_id, f"[OK] Text extracted: {len(text)} characters", "success")
    except Exception as e:
        logger.error(f"[ERROR] Text loading failed: {e}")
        if session_id:
    ...
    try:
        ...
        logger.info(f"[OK] Text chunked successfully: {len(chunks)} chunks")
        if session_id:
        ...
        logger.error(f"[ERROR] Text chunking failed: {e}")
        if session_id:
    ...
    try:
        ...
        logger.info(f"[OK] Document ingestion completed for {filename}")
        if session_id:
            progress_tracker.add_step(session_id, f"[OK] Document stored successfully", "success")
    except Exception as e:
        logger.error(f"[ERROR] Vector store addition failed: {e}")
        if session_id:
